...
export default async function migrateData() {
  ...
  await sequelize.query('alter table "Mentorships" drop column if exists "coachId"');
  await sequelize.query('alter table "groups" drop column if exists "coachingMentorshipId"');
  await sequelize.query('alter table "ChatMessages" drop column if exists "threadId"');
  await sequelize.query('alter table "Mentorships" drop column if exists "internalChatThreadId"');

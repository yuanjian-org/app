...
function Applicant({
  ...
  if (matches.length > 1) {
    console.error(
      `Applicant id ${applicant.id} has multiple matching ` + `interviews ids:`,
    );
            ...
            <FormControl>
              ...
              <UserSelector
                ...
                initialValue={
                  !interview
                    ? []
                    : interview.feedbacks.map((f) => f.interviewer)
                }
              />
            </FormControl>
            <FormControl>
        ...
        <Table>
          ...
          <Tbody>
            {calibrations
              ...
              .sort((c1, c2) =>
                moment(c2.createdAt).diff(moment(c1.createdAt), "seconds"),
              )
              .map((c) => {
                return (
                  <Tr key={c.id}>
                    ...
                    <Td>{c.createdAt && prettifyDate(c.createdAt)}</Td>
                  </Tr>
                );
              })}
          </Tbody>

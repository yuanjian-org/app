...
export default widePage(() => {
  const { data } = trpcNext.mentorships.listOngoingRelationalMentorships
    .useQuery();
  ...
  return !sorted ? <Loader /> : <TableContainer>
    <Table size="sm">
      <Thead>
        <Tr>
          <Th>学生</Th>
      ...
      <Tbody>
        {sorted.map(m => <Row key={m.id} mentorship={m} h={h} max={max} />)}
      </Tbody>
    </Table>
...
function Row({ mentorship: m, h, max }: {
  ...
  return <Tr _hover={{ bg: "white" }}>
    ...
    {s && _.range(bucketsPerMeeting).map(i => {
      const count = h.get(buckets(s)[i]);
      return <Td key={i}>
        {count === max ?
          <Text fontWeight="bold" color="red">{count}</Text> : count}
      </Td>;
    })}
  </Tr>;
}

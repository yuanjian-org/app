...
class ApiEnvDeprecated {
  ...
  INTEGRATION_AUTH_TOKEN: string | undefined =
    process.env.INTEGRATION_AUTH_TOKEN;
  ...
  hasTencentMeeting() {
    return this.TM_SECRET_KEY.length != 0 && this.TM_SECRET_KEY != "DUMMY";
  }
}
...
validateOrReject(apiEnv).catch((errors) => {
  ...
  throw Error(
    "Found invalid env variables. See console messages above for detail",
  );
});

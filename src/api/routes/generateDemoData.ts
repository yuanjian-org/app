...
export async function generateDemoData(t: Transaction) {
  ...
  if (group1) {
    ...
    // to after group2 is created and use mentor1 instead of admin as the
    // author.
...
async function createProjects(t: Transaction) {
  ...
  const p1 = await db.Project.create(
    {
      creatorId: id(mentor1),
      title: "第三代半导体在光催化环境下的氧化、磨损机理研究",
      profile: {
        Background: "半导体材料在特殊环境下的应用研究日益重要。",
        "Challenge Description":
          "本课题致力于探索第三、四代半导体在光催化环境下的摩擦与磨损机理，期望能在源头创新领域取得重大突破。",
        Video: "https://example.com/video1.mp4",
        学生画像要求:
          "物理、材料或相关专业背景，对科研有热情，具备基础实验能力。",
      },
      requireLogin: true,
      isPublished: true,
    },

...
import { zMentorSelection, zMentorSelectionBatch } from "shared/MentorSelection";
import {
 ...
 */
async function getDraftBatch(myId: string, mentorId: string | undefined,
  ...
  .mutation(async ({ ctx: { user }, input }) => 
{
  await sequelize.transaction(async transaction => {
    ...
    for (const { mentorId, order } of input) {
      const [cnt] = await db.MentorSelection.update({ order }, {
        where: {
          batchId: batch.id,
          mentorId,
        },

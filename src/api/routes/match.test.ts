...
describe("match.ts", () => {
  describe("applyInitialSolverOutputImpl", () => {
    ...
    it("should create transactional mentorship and match feedback successfully", async () => {
      ...
      expect((menteeFeedback!.feedback as Extract<import("shared/MatchFeedback").MatchFeedback, { type: "Mentee" }>).mentors).to.deep.equal([
        { id: mentor1.id },
        { id: mentor2.id },
      ]);
      ...
      expect((mentor1Feedback!.feedback as Extract<import("shared/MatchFeedback").MatchFeedback, { type: "Mentor" }>).mentees).to.deep.equal([
        { id: mentee.id },
      ]);
      ...
      expect((mentor2Feedback!.feedback as Extract<import("shared/MatchFeedback").MatchFeedback, { type: "Mentor" }>).mentees).to.deep.equal([
        { id: mentee.id },
      ]);
    });
  });

...
export default function GroupBar({
  ...
  const launchMeeting = async (groupId: string) => {
    ...
    try {
      ...
      if (!link) {
        ...
        // Mitigate reverse tabnabbing vulnerability while retaining reference for fallback check.
        if (w) {
          w.opener = null;
        }
  ...
  return (
    <SimpleGrid
      ...
      {...rest}
    >
      {showMeetingQuotaWarning && (
        <MeetingQuotaWarning
          onClose={() => setShowMeetingQuotaWarning(false)}
        />
      )}
    ...
    <>
      ...
      <ModalWithBackdrop isOpen={declined} onClose={onClose}>
        <ModalContent>
          ...
          <ModalBody>
            <Text>
              您已拒绝使用本平台的会议功能。工作人员会与您取得联系，商量解决方案。
            </Text>
          </ModalBody>
          <ModalFooter>
...
function MeetingQuotaWarning({ onClose }: { onClose: () => void }) {
  return (
...
function UserChips({
  ...
  const abbrOnMobile =
    (abbreviateOnMobile === undefined || abbreviateOnMobile) &&
  ...
  return (
    <>
      {/* Abbreviated mode */}
      <AvatarGroup
        // No reason to display a "+1" avatar.
        max={displayUsers.length > 4 ? 3 : 4}
        display={{
      ...
      >
        {displayUsers.map((user) => (
          <Avatar key={user.id} name={user.name || undefined} />
        ))}
      </AvatarGroup>

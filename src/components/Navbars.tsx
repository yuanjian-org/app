...
import { IconButton, Tooltip, Box, useDisclosure } from "@chakra-ui/react";
...
export default function Navbars({ children }: { children: ReactNode }) {
  const { isOpen, onOpen, onClose } = useDisclosure();
  const [autosaveState, setAutosateState] =
    useState<AutosaveState>(initialState);
...
function SidbarIconRedDot() {
  ...
  const hasUnreadChatMessages = useUnreadChatMessages(
    mentorships
      ?.filter((m) => showRedDotForMentorship(m))
      .map((m) => m.mentee.id) ?? [],
  );
...
function AutosaveIndicatorForMobile({
  autosaveState,
}: {
...
function AutosaveIndicatorForDesktop({
  autosaveState,
}: {
  autosaveState: AutosaveState;
}) {

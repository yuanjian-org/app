import { Td, Link, HStack, Tooltip, Badge } from "@chakra-ui/react";
...
export default function MergeTokenCell({
  user,
  refetch,
}: {
  ...
  return (
    <Td>
      <HStack spacing={2}>
        <SendMergeToken userId={user.id} email={user.email} refetch={refetch} />
...
function SendMergeToken({
  ...
  return !canIssueMergeToken(email) ? (
    <></>
  ) : (
    <Link onClick={() => setConfirming(true)}>
      <MdMerge />
      {confirming && (
        <ConfirmationModal

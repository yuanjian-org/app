...
function BookingButtonAndModal({ user }: { user: MinUser }) {
  ...
  return (
    <>
      ...
      {booking && (
        <MentorBookingModal mentor={user} onClose={() => setBooking(false)} />
      )}
    </>
  );
}
      ...
      <Table variant="unstyled">
        <Tbody>
          {visibleUserProfileFields.map((fl, idx) => {
            let computedLabel = fl.label ?? fl.field;
            if (fl.field === "常用联系方式" && p[fl.field]) {
              computedLabel = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(
                p[fl.field] || "",
              )
                ? "联系方式（Email）"
                : "联系方式（微信号）";
            ...
            return (
              <React.Fragment key={idx}>
                <ProfileRow label={computedLabel} content={p[fl.field]} />

...
describe("ExamsRequired component", () => {
  ...
  it("renders comms exam link correctly", () => {
    const { getByText } = render(
      <ExamsRequired commsExamRequired={true} />
    );
    expect(getByText("《学生通信原则》自学与评测")).to.exist;
    expect(getByText(/即可看到面试信息/)).to.exist;
    expect(getByText(/面试官/)).to.exist;
  });
  ...
  it("renders interview exam link correctly", () => {
    ...
    expect(getByText("面试官自学与评测")).to.exist;
  });

  it("renders handbook exam link correctly", () => {
    const { getByText } = render(
      <ExamsRequired handbookExamRequired={true} />
    );
    expect(getByText("《社会导师手册》自学与评测")).to.exist;
  });
  ...
  it("renders custom actionText and roleText", () => {
    const { getByText } = render(
      <ExamsRequired commsExamRequired={true} actionText="即可参与" roleText="测试用户" />
    );
    expect(getByText(/即可参与/)).to.exist;
    expect(getByText(/我们邀请测试用户每年/)).to.exist;
  });
  ...
  it("renders interview specific paragraph if interview exam is required", () => {
    ...
    expect(getByText(/导师面试与学生面试的原则一致/)).to.exist;
  });
  ...
  it("does not render interview specific paragraph if interview exam is not required", () => {
    ...
    expect(queryByText(/导师面试与学生面试的原则一致/)).to.be.null;
  });
});
